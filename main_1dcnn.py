...
if __name__ == '__main__':
    ...
    train_map = {
        ...
        11: 1.22 * np.array([10, 0]),
        12: 1.22 * np.array([11, 0])
    }
    ...
    test_map = {
        ...
        11: 1.22 * np.array([10, 0]) + 0.3 * np.array([1, 0]),
        12: 1.22 * np.array([11, 0]) + 0.3 * np.array([1, 0])
    }

    loc_num = 12
    loc_list = np.arange(1, loc_num + 1)
    loc_list_num = len(loc_list)
    f_train_path = "dataset/lobby/train/"
    ...
    f_test_path = "dataset/lobby/test/"
    ...
    for count in range(1):
        model = Sequential()
        model.add(Conv1D(filters=128, kernel_size=5, input_shape=(num_timesteps, num_features), activation='relu'))
        model.add(Conv1D(filters=128, kernel_size=5, input_shape=(num_timesteps, num_features), activation='relu'))
        model.add(GlobalMaxPooling1D())
        model.add(Dense(8, activation='relu'))
        model.add(Dense(y_train.shape[1], activation='softmax'))
        model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
        es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=24)
        ...
        print("rmse = {:.2f}".format(pos_err_rmse))

        # # cdf
        # np_hist, np_bins = np.histogram(pos_err, bins=np.arange(0, 10, 0.1), density=True)
        # dx = np_bins[1] - np_bins[0]
        # cdf = np.cumsum(np_hist) * dx
        # cdf_x = np_bins[1:]
        # cdf_y = cdf
        # joblib.dump(cdf_x, f_save_path + "cdf_1dcnn_x" + "_c" + str(count) + ".save")
        # joblib.dump(cdf_y, f_save_path + "cdf_1dcnn_y" + "_c" + str(count) + ".save")
